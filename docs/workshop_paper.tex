...
core VSA primitives---binding, bundling, permutation, similarity,
inverse---are redefined directly on distributions and propagate
closed-form moments under the standard independence assumptions of
hyperdimensional computing (HDC). We derive explicit formulas for
Gaussian, Dirichlet, and mixture hypervector types, show how
deterministic HDC is recovered as the zero-variance limit, and show how
three post-hoc mechanisms---temperature scaling, split-conformal
prediction with APS scores, and multi-restart MCMC resonator
factorisation---convert any PVSA classifier into a calibrated predictor
with coverage guarantees. Empirically, PVSA classifiers match or
outperform the strongest public HDC baseline (Torchhd; Heddes et al.,
2023) on all five standard tabular benchmarks (mean $+3.94$pp,
MNIST $+8.9$pp), reduce Expected Calibration Error by $5$--$25\times$
on MNIST-scale workloads, and achieve the nominal $1 - \alpha$ coverage
on every dataset. A full implementation is released as the open-source
\texttt{bayes-hdc} library at \url{https://github.com/rlogger/bayes-hdc}.
\end{abstract}
...
(\texttt{python benchmarks/benchmark\_calibration.py}); paper-ready
reliability diagrams and coverage curves for every dataset live at
\texttt{benchmarks/figures/} and are regenerated by
\texttt{python benchmarks/generate\_figures.py}. All experiments also
run inside a reproducible Docker image, invoked via
\texttt{make docker-bench} with a volume-mounted results directory.

Sample figures: Figure~\ref{fig:reliability-digits} shows the
reliability diagram on the UCI digits task; Figure~\ref{fig:coverage-wine}
shows empirical conformal coverage vs the target $1 - \alpha$ on wine.
Both illustrate that PVSA's calibration and coverage claims hold
out-of-the-box, without per-dataset tuning.

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{../benchmarks/figures/reliability_digits.pdf}
\caption{Reliability diagram for the PVSA temperature-calibrated
classifier on UCI digits. Blue bars are empirical accuracy per
confidence bin; red overlays are the confidence--accuracy gaps.
The classifier is well-calibrated across the confidence range.}
\label{fig:reliability-digits}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{../benchmarks/figures/coverage_wine.pdf}
\caption{Conformal coverage on wine. Empirical coverage (blue, left
axis) tracks the target $1 - \alpha$ (dashed); mean prediction-set
size (red, right axis) grows smoothly as the coverage target tightens.
PVSA delivers the finite-sample guarantee without magic thresholds.}
\label{fig:coverage-wine}
\end{figure}
