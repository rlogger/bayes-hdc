...
\ShortHeadings{Bayes-HDC: A Bayesian Framework for Hyperdimensional Computing}{Singh}

\firstpageno{1}

\begin{document}

\title{Bayes-HDC: A Bayesian Framework for \\ Hyperdimensional Computing and Vector Symbolic Architectures}
...
\begin{abstract}
We introduce Bayes-HDC, the first open-source library for hyperdimensional computing (HDC) in which every hypervector can carry a distribution. Existing HDC libraries represent symbols as single points in $\mathbb{R}^d$; Bayes-HDC represents them as posteriors, and propagates the posterior forward through the full VSA algebra---binding, bundling, similarity, and retrieval---so that downstream classification and retrieval inherit calibrated uncertainty. The library ships a Gaussian hypervector type with closed-form moment propagation under element-wise product (MAP-style binding) and normalised sum (bundling), together with a closed-form KL divergence suitable as a variational objective. A complete deterministic VSA foundation---eight classical models (BSC, MAP, HRR, FHRR, BSBC, CGR, MCR, VTB), five encoders, five learning models, three memory modules, four symbolic data structures, and a capacity-and-noise analysis toolkit---underpins the Bayesian layer and provides a baseline for comparison. All operations are JIT-compilable and run unchanged on CPUs, GPUs, and TPUs via JAX's XLA backend. The library is available under the MIT license at \url{https://github.com/rlogger/bayes-hdc}.
\end{abstract}

\begin{keywords}
Bayesian Hyperdimensional Computing, Probabilistic Vector Symbolic Architectures, Uncertainty Quantification, Calibrated Retrieval, JAX
\end{keywords}
