...
\begin{abstract}
We introduce \emph{Probabilistic Vector Symbolic Architectures} (PVSA), a new framework in which every hypervector is a posterior distribution rather than a point, and every VSA primitive---binding, bundling, similarity, and retrieval---propagates the posterior's moments in closed form. Bayes-HDC is the first open-source library implementing PVSA. It ships two distribution types---diagonal-covariance Gaussian hypervectors and Dirichlet hypervectors over the probability simplex---each with closed-form moment propagation under the core VSA operations and closed-form Kullback--Leibler divergences suitable as variational objectives. On top of the distributional algebra, Bayes-HDC provides two uncertainty-quantification wrappers: a log-space L-BFGS temperature calibrator that reduces Expected Calibration Error by $5$--$25\times$ on standard HDC datasets, and a split-conformal classifier with the Adaptive Prediction Sets nonconformity score that returns prediction sets with provable marginal coverage at level $1 - \alpha$. A complete deterministic VSA foundation---eight classical models (BSC, MAP, HRR, FHRR, BSBC, CGR, MCR, VTB), five encoders, five learning models, three memory modules, four symbolic data structures, and a capacity-and-noise analysis toolkit---serves as a baseline for comparison and the substrate on which PVSA is built. Head-to-head benchmarks against Torchhd on five datasets (iris, wine, breast-cancer, digits, MNIST) show accuracy parity and calibration-loss reductions of up to $25\times$. All operations are JIT-compilable and run unchanged on CPUs, GPUs, and TPUs via JAX's XLA backend. The library is available under the MIT license at \url{https://github.com/rlogger/bayes-hdc}.
\end{abstract}
...
Bayes-HDC addresses the gap by lifting every hypervector to a distribution. The primary contribution is a Bayesian layer in which binding, bundling, similarity, and retrieval are defined directly on distributions rather than on samples, giving the user first-class control over uncertainty. The layer is built on JAX \citep{jax2018github} so that all operations are JIT-compilable, vectorisable via \texttt{vmap}, and differentiable via \texttt{grad}, which in turn enables variational learning of codebooks. Concretely, this paper contributes:

\begin{enumerate}
\item \textbf{A Gaussian hypervector type} (\texttt{GaussianHV}) with closed-form moment propagation under element-wise product and normalised sum, together with a closed-form Kullback--Leibler divergence that serves as a variational objective. Deterministic hypervectors are recovered as the zero-variance limit.

\item \textbf{An uncertainty-aware similarity API} comprising expected cosine similarity at the moment-matched Gaussian and the exact first-order variance of the dot product, enabling calibrated retrieval without Monte Carlo.

\item \textbf{A complete deterministic VSA foundation}---eight algebraic models (BSC, MAP, HRR, FHRR, BSBC, CGR, MCR, VTB) with a uniform pytree-native API, five data encoders, five learning models including clustering, three associative memory modules, four symbolic data structures, an analysis toolkit for capacity, signal-to-noise ratio, participation ratio, and retrieval confidence, and a resonator-network primitive for factorisation.

\item \textbf{XLA-backed portability} across CPUs, GPUs, and TPUs without code changes, and a reproducible benchmark harness that compares Bayes-HDC against Torchhd on identical workloads.
\end{enumerate}

Sections \ref{sec:system} through \ref{sec:examples} describe the library architecture and provide usage examples, Section \ref{sec:related} positions the contribution relative to prior software and calibration literature, and Section \ref{sec:conclusion} outlines future directions on the research roadmap.

\section{The Bayes-HDC System}
\label{sec:system}

\subsection{Design Principles}

Bayes-HDC is built on four principles:
...
Bayes-HDC consists of eight modules. Table~\ref{tab:modules} summarizes the components.

\begin{table}[t]
\centering
\caption{Bayes-HDC module overview.}
\label{tab:modules}
\begin{tabular}{lll}
\toprule
...
\begin{table}[t]
\centering
\caption{VSA models implemented in Bayes-HDC. All share the interface: \texttt{create}, \texttt{random}, \texttt{bind}, \texttt{bundle}, \texttt{inverse}, \texttt{similarity}.}
\label{tab:models}
...
Four symbolic data structures are implemented directly on top of the core VSA primitives, following the constructions in the HDC literature \citep{kanerva2009hyperdimensional,kanerva2010dollar,plate1995holographic}:
...
Table~\ref{tab:comparison} compares Bayes-HDC with existing HDC libraries.
...
Bayes-HDC introduces Probabilistic Vector Symbolic Architectures, an algebra in which every hypervector is a posterior distribution and every VSA primitive propagates the posterior's moments in closed form. No prior HDC library offers this framework, nor its constituent pieces: closed-form distributional binding and bundling (\texttt{GaussianHV}, \texttt{DirichletHV}), L-BFGS-in-log-space temperature calibration fit on held-out data, split-conformal prediction with the APS nonconformity score for marginal-coverage guarantees, or calibration-metric primitives (ECE, MCE, Brier, reliability curves) as JIT-compilable operations. The deterministic VSA layer is implemented from primary research---Kanerva (1997, 2009), Plate (1995), Gayler (2003), and the related-model surveys of \citet{kleyko2022survey1,kleyko2022survey2}---rather than derived from other library implementations; Torchhd \citep{heddes2023torchhd}, hdlib \citep{cumbo2023hdlib}, and PyBHV \citep{vandervorst2023pybhv} are included as empirical baselines for head-to-head comparison.
...
Bayes-HDC is open source under the MIT license at \url{https://github.com/rlogger/bayes-hdc}.
