...
`bayes-hdc` is a JAX [@jax2018github] library for hyperdimensional computing (HDC) and vector symbolic architectures (VSA) [@gayler2003vsa; @kanerva2009hyperdimensional] that pairs a faithful re-implementation of eight classical VSA models — BSC, MAP, HRR, FHRR, BSBC, CGR, MCR, and VTB — with a built-in *probabilistic* layer. The probabilistic layer, **PVSA** (*Probabilistic Vector Symbolic Architectures*), introduces Gaussian and Dirichlet hypervector types whose first and second moments propagate analytically through `bind`, `bundle`, `permute`, and `cleanup`; closed-form Kullback–Leibler divergences between hypervector posteriors; reparameterisation gradients for variational training of codebooks and classifier posteriors; group-theoretic equivariance verifiers for the cyclic-shift action of $\mathbb{Z}/d$; temperature-scaling calibration [@guo2017calibration]; and split-conformal prediction sets with finite-sample coverage guarantees [@romano2020classification]. Every type is registered as a JAX pytree, so `jit`, `vmap`, `grad`, `pmap`, and `shard_map` compose with every operation on CPU, GPU, and TPU without user-side flattening. The library ships 14 application examples spanning EMG gesture recognition, EEG seizure detection, character-trigram language identification, and the Kanerva "Dollar of Mexico" analogical-mapping benchmark, alongside 498 unit tests at 97 % line coverage.

# Statement of need

Existing HDC software is dominated by `TorchHD` [@heddes2023torchhd], a mature PyTorch library that covers the same eight VSA models. `TorchHD` does not, however, expose a probabilistic layer: its hypervectors carry no posterior variance, its classifiers have no calibration step, and it has no conformal-prediction module. Two needs follow.
...
The HDC ecosystem has four maintained libraries. `TorchHD` [@heddes2023torchhd] is the dominant general-purpose library; PyTorch backend, ~370 GitHub stars at the time of writing, JMLR-published, the same eight VSA models. `hdlib` [@cumbo2023hdlib] targets bioinformatics and cheminformatics on a NumPy backend with a domain-application focus rather than research-primitive depth. `vsapy` is a NumPy library specialising in number-line and circular encodings. `NengoSPA` [@bekolay2014nengo] is the spiking-neuron lineage of HDC from the Eliasmith group, embedded in the Nengo neural simulator; it is biologically faithful but disjoint from machine-learning workflows.

`bayes-hdc` differs from each on a different axis: against `TorchHD`, on the JAX backend and the probabilistic layer; against `hdlib` and `vsapy`, on JIT performance, autodifferentiation, GPU and TPU support, and the size of the primitive set; against `NengoSPA`, on machine-learning integration. To our knowledge, no JAX-native HDC library was publicly available before this one.
